...
export default function AdminDashboard({ user, setView }) {
  ...
  const NAV = [
    { id: 'pending',  icon: '⏳', label: 'Pending',     get count() { return stats.pending;  } },
    { id: 'listings', icon: '📦', label: 'Listings',    get count() { return stats.listings; } },
    { id: 'add',      icon: '➕', label: 'Add Listing', count: 0 },
    { id: 'bookings', icon: '📅', label: 'Bookings',    get count() { return stats.bookings; } },
    { id: 'messages', icon: '💬', label: 'Messages',    get count() { return stats.messages; } },
  ];
    ...
    <div style={{ position: 'absolute', top: 'calc(100% + 8px)', left: 0, width: '260px', background: '#0d1520', border: '2px solid #1e2a3a', borderRadius: '14px', zIndex: 9999, overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,0.7)' }}>
      ...
      <nav style={{ padding: '8px' }}>
        {NAV.map(item => (
          <button key={item.id} onClick={() => { setTab(item.id); setMenuOpen(false); }}
            style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 13px', marginBottom: '2px', borderRadius: '9px', background: tab === item.id ? '#4dd4ac' : 'transparent', color: tab === item.id ? '#000' : '#4dd4ac', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.87rem', fontWeight: tab === item.id ? '700' : '500', transition: 'background 0.15s' }}
      ...
      <div style={{ padding: '10px 12px', borderTop: '2px solid #1e2a3a', display: 'flex', flexDirection: 'column', gap: '7px' }}>
        <div style={{ display: 'flex', gap: '7px' }}>
          ...
          <button onClick={() => { load(); setMenuOpen(false); }} style={{ flex: 1, padding: '8px', background: 'rgba(77,212,172,0.08)', border: '1px solid #1e2a3a', borderRadius: '8px', color: '#4dd4ac', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.74rem', fontWeight: '600' }}>
            ↻ Refresh
          </button>
        </div>
