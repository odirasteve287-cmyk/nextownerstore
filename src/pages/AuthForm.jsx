...
export default function AuthForm({ type: initialType, setView }) {
  ...
  // ── UPDATED: redirectTo ensures Google shows "nextownerstore.sale" ──
  const handleGoogleLogin = async () => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: 'https://nextownerstore.sale/auth/callback',
      },
    });
        ...
        <div style={{ padding: '28px 32px 32px' }}>
          ...
          <button
            ...
            style={{
              ...
              gap: '12px', padding: '12px', marginBottom: '20px',
              background: '#1a2332', border: '1.5px solid #1e2d3d', borderRadius: '6px',
              color: '#94a3b8', fontSize: '14px', cursor: 'pointer',
              transition: 'border-color 0.2s, background 0.2s',
            }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = '#0bbfaa'; e.currentTarget.style.background = '#1e2d3d'; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = '#1e2d3d'; e.currentTarget.style.background = '#1a2332'; }}
          >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
            <div style={{ flex: 1, borderTop: '1px dashed #1e2d3d' }} />
